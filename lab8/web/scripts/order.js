document.addEventListener('DOMContentLoaded', () => {
  ...
  function displaySelectedDishes(selectedDishes, container) {
    ...
    Object.entries(selectedDishes).forEach(([category, dish]) => {
      if (dish) {
        ...
        // Убираем кнопку "Добавить", так как блюдо уже выбрано
        const button = document.createElement('button');
        button.textContent = 'Удалить'; // Кнопка для удаления
  
        // Кнопка "Удалить" добавляется для удаления блюда
  ...
  function loadSelectedDishes() {
    ...
    // Обновляем объект selectedDishes
  ...
  function updateOrderDisplay() {
    ...
    // Обновление отображения каждого блюда
  ...
  function updateDishDisplay(selector, dish) {
    ...
    if (dishElement) {
      if (dish) {
        // Если блюдо выбрано, отображаем его имя и цену
        dishElement.textContent = `${dish.name} ${dish.price}₽`;
      } else {
        // Если блюдо не выбрано, отображаем "Не выбрано"
